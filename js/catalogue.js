...
function renderProducts(container, products) {
  ...
  // ✅ Function to render next set of products with fade-in
  function renderNextBatch() {
    const nextProducts = products.slice(visibleCount, visibleCount + perPage);

    nextProducts.forEach((product) => {
      const col = document.createElement("div");
      col.className = "col-md-3 col-sm-6 product-card mb-4 fade-in"; // <-- fade class
      ...
      // Remove fade-in class after animation finishes (so it won’t repeat)
      setTimeout(() => {
